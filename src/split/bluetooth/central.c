...
THIS IS A PATCHED VERSION of src/split/bluetooth/central.c

    All patched lines are inside code block:
...
struct k_work_delayable suspend_scanning_work;
static bool suspend_rescan = false;

static int8_t peripheral_connected_count() {
    int8_t conn_count = 0;
...
static void advance_min_active_peripheral_counter() {
    int8_t conn_count = peripheral_connected_count();
    if (min_active_peripheral_count < conn_count) {
        min_active_peripheral_count = conn_count;
        LOG_WRN("Advance min active peripheral count: %d", min_active_peripheral_count);
    }
}

static int stop_scanning(void);

static void suspend_scanning(struct k_work *work) {
    // If all the (had-been-connected-on-init) devices are connected, there is no need to scan.
    int8_t conn_count = peripheral_connected_count();
    if (conn_count >= min_active_peripheral_count) {
        LOG_WRN("%d device(s), scanning is suspending. (>= %d initial peripheral)", 
                conn_count, min_active_peripheral_count);
        suspend_rescan = true;
        if (is_scanning) {
            stop_scanning();
        }
...
#if CONFIG_ZMK_SPLIT_BLE_HAS_CONT_SCAN_WINDOW
    advance_min_active_peripheral_counter();
    int8_t conn_count = peripheral_connected_count();
    if (conn_count >= 1) {
        LOG_WRN("Schedule to suspend peripheral scanning in %d second", 
...
#if IS_ENABLED(CONFIG_ZMK_SPLIT_BLE_PREF_WEAK_BOND)
static void split_central_security_changed(struct bt_conn *conn, bt_security_t level, enum bt_security_err err) {
    char addr[BT_ADDR_LE_STR_LEN];
    bt_addr_le_to_str(bt_conn_get_dst(conn), addr, sizeof(addr));
    if (err) {
        struct bt_conn_info info;
